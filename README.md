...
- **End users do not need Node/npm/pnpm.** The installer bundles the Electron runtime and all renderer dependencies. They only run the `.exe` produced below.
- You (the builder) need Node + pnpm and Windows build tools (Visual Studio Build Tools or `windows-build-tools`). Then run these from the repo root:
  ```bash
  pnpm install
  pnpm --filter desktop build
  pnpm --filter desktop package:win
  ```
- Output: `apps/desktop/release/Mcord-Setup-<version>.exe`. Share this file with users; the installer lets them choose an install folder and places shortcuts.
...
- **Only the host needs Node/pnpm and Docker.** Friends who only chat with the installer do not install any tooling.
